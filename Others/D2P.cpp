...
2010.07.15 increase MAXseq to 100Kb			*/
...
{   int num_seq=0;
    ...
    cout<< "This program translate all fasta formated DNA sequence into 3 or 6 frames"<<endl;
    cout<< "------------  copyright Lei Zhou     ------------"<<endl<<endl;
    ...
    infile.open(dfile);
    if (!infile)
    {   cout<<"Can not open DNA file"<<endl;
        getch();
        return 0;
    }
    // name output peptide file
    cout<<"Protein file name: ";
    cin>>pfile;
    if (&pfile==NULL)
    {   p=".pep.txt";
        pfile= strcat (dfile, p);
    }

    outfile.open(pfile);
    outfile<<"DNA file:"<<dfile<<endl;

    //get REVERSE parameter
    ...
    while (!infile.eof())
    {   dseq=Get_Lib_Seq ();
        Translation (&dseq->name[1], dseq->seq, 0);
        if (REVERSE==1)
            Translation (&dseq->name[1], dseq->r_seq, REVERSE);
    	num_seq++;
        if (num_seq%1000==0)
        {   cout<<num_seq<<endl;
            cout<<dseq->name<<endl;
...
void Translation (char name[], char seq[], bool rev)
{   char ppseq[MAXSEQ/3];
    ...
    {   if (rev==0)
        ...
        if (rev==1)
        outfile<<">"<<"R_"<<f<<"  "<<name<<endl;     // R for reverse
        int j=0;
    	int len=strlen(seq);
